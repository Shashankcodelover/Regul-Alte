...
log = logging.getLogger("regulaite.rag")
...
class InMemoryRAG:
    ...
    Usage:
        rag = InMemoryRAG()
        rag.index(clauses)          # list of {number, text} dicts
        result = rag.verify(claim)  # {verified, confidence, source_clause}
    """
    ...
    def __init__(self):
        ...
        self._use_st = False        # whether sentence-transformers is available
        self._tfidf: Optional[TFIDFVectorizer] = None
        self._store: List[Dict[str, Any]] = []   # [{clause, vector}]
        self._indexed = False
    ...
    def _load_model(self):
        ...
        try:
            ...
            self._use_st = True
            log.info("RAG: using sentence-transformers all-MiniLM-L6-v2")
        except Exception as exc:
            log.warning(f"RAG: sentence-transformers unavailable ({exc}), using TF-IDF fallback.")
            self._use_st = False
    ...
    def index(self, clauses: List[Dict[str, Any]]) -> int:
        ...
        if not self._use_st:
            # Fit TF-IDF on all clause texts
        ...
        self._indexed = True
        log.info(f"RAG: indexed {len(self._store)} clauses.")
        return len(self._store)

    def verify(self, claim: str) -> Dict[str, Any]:
        """
        Find the most similar clause to the claim.
        Returns {verified, confidence, source_clause, source_number}.
        verified=True if cosine similarity >= SIMILARITY_THRESHOLD.
        """
        if not self._indexed or not self._store:
            return {"verified": False, "confidence": 0.0, "source_clause": "", "source_number": "?"}
        ...
        return {
            ...
            "source_clause": best_clause.get("text", "")[:200] if best_clause else "",
            "source_number": best_clause.get("number", "?") if best_clause else "?",
        }
